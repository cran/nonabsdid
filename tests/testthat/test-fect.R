...
test_that("fect tidier extracts time, att, and CI bounds", {
  ...
  expect_equal(out$method, rep("IFE", 7))
  expect_equal(out$time, -3:3)
...
test_that("fect tidier auto-labels FE / IFE / MC from $method", {
  ...
  expect_equal(unique(as_nabs_event_study(f_fe )$method), "FE")
  expect_equal(unique(as_nabs_event_study(f_ife)$method), "IFE")
  expect_equal(unique(as_nabs_event_study(f_mc )$method), "MC")
})
...
test_that("fect tidier handles legacy / unknown method strings", {
  ...
  expect_equal(unique(as_nabs_event_study(f_legacy)$method), "Imputation")

  # An object built without a method slot (very old version) falls back to "IFE".
  f <- make_mock_fect()
  f$method <- NULL
  expect_equal(unique(as_nabs_event_study(f)$method), "IFE")
